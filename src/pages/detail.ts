...
export function detailPageFrom(source: PageSource, date: string, alternates: string[] = []): DetailPage {
	...
	return {
		...
		alternates,
		comics,
		rerunOf: source.reruns.get(date) ?? null,
...
/**
 * A rerun day has no strip of its own; it points at the one that ran again. `data-original-date`
 * is what the view reads to light up that strip's cell in the grid while the link is hovered.
 */
function buildRerunHtml(originalDate: string): string {
	return `<p class="detail-rerun">On this day, the strip from <a class="detail-rerun-link" href="${addressOf(buildComicPath(originalDate))}" data-original-date="${originalDate}">${formatLongDate(originalDate)}</a> was rerun</p>`;
}
...
export function buildDetailHtml(page: DetailPage, canGoBack: boolean): string {
	...
	const bodyHtml = page.rerunOf
		? buildRerunHtml(page.rerunOf)
		: comics.length === 0
