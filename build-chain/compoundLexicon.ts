...
// Closed forms whose equivalence is part of the search contract even when the corpus does not
// contain enough open examples to infer them statistically.
const REQUIRED = new Map([["snowball", ["snow", "ball"]]]);
...
export function buildLexicon(projectDir: string): Map<string, string[]> {
	...
	for (const [closed, parts] of REQUIRED) lexicon.set(closed, parts);
